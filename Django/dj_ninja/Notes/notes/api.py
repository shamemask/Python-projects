...
from .schemas import NoteIn, NoteOut, NoteUpdTitle, NoteUpdSccss, NoteUpdFvrt, CategoryIn, CategoryOut
...
@api.patch("/notes/{note_id}/title/", tags=['Заметки'])
def update_note_title(request, note_id: int, payload: NoteUpdTitle):
    note = get_object_or_404(Note, id=note_id)
...
@api.patch("/notes/{note_id}/success/", tags=['Заметки'])
def update_note_success(request, note_id: int, payload: NoteUpdSccss):
    note = get_object_or_404(Note, id=note_id)
    for attr, value in payload.dict().items():
        setattr(note, attr, value)
    note.save()
...
@api.patch("/notes/{note_id}/favorite/", tags=['Заметки'])
def update_note_favorite(request, note_id: int, payload: NoteUpdFvrt):
    note = get_object_or_404(Note, id=note_id)
    for attr, value in payload.dict().items():
        setattr(note, attr, value)
    note.save()
